...
const LoginPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Ocurrió un error al iniciar sesión. Por favor intenta de nuevo.');
      console.error(err);
    } finally {
      setIsLoading(false);
    }
  ...
  const handleGoogleAuth = async () => {
    try {
      ...
      const response = await authService.googleAuth(mockGoogleData);
      localStorage.setItem('token', response.token);
      localStorage.setItem('user', JSON.stringify(response));
      navigate('/profile');
    } catch (error: any) {
      setError(error.response?.data?.message || 'Error en autenticación con Google');
    }
  };
  ...
  const handleFacebookAuth = async () => {
    try {
      ...
      const response = await authService.facebookAuth(mockFacebookData);
      localStorage.setItem('token', response.token);
      localStorage.setItem('user', JSON.stringify(response));
      navigate('/profile');
    } catch (error: any) {
      setError(error.response?.data?.message || 'Error en autenticación con Facebook');
    }
  };
  ...
  const handleForgotPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setForgotMessage(error.response?.data?.message || 'Error al enviar email de recuperación');
    } finally {
          ...
          <div className="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
            {forgotMessage && (
              <div className="mb-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-sm text-blue-700 whitespace-pre-line">{forgotMessage}</p>
              </div>
            )}
